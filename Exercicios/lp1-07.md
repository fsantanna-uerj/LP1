...
Defina um tipo para representar as classes e propriedades de animais
vertebrados:
...
Crie uma `union` que permita representar um ponto das duas formas de maneira
intercambiável, ex.:
...
- Crie uma função `preenche` que receba um ponteiro para uma variável do tipo
  criado (pode ser uma `struct` ou `union`) e leia os campos para a variável
  passada.

## 18.

Altere o exemplo `13` para incluir propriedades em comum a todas as classes de
